...
import { getApprovedClubs } from "../../services/clubService";
...
function ClubCard({ club }) {
  ...
  return (
    <article
      role="button"
      tabIndex={0}
      onClick={() => navigate(`/club/apply/${club.id}`)}
      onKeyDown={(event) => {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {isError && (
          <div className="flex min-h-80 flex-col items-center justify-center gap-3 rounded-2xl border border-red-100 bg-white px-6 text-center shadow-sm">
            ...
            <div>
              <p className="font-medium text-gray-700">동아리 목록을 불러오지 못했습니다.</p>
              <p className="mt-1 text-sm text-gray-400">잠시 후 다시 시도해주세요.</p>
            </div>
            <button
              type="button"
              onClick={() => refetch()}
              className="mt-1 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700 cursor-pointer"
            >
              다시 시도
            </button>
          </div>
        ...
        {!isLoading && !isError && clubs.length === 0 && (
          <div className="flex min-h-80 flex-col items-center justify-center gap-3 rounded-2xl border border-gray-100 bg-white px-6 text-center shadow-sm">
            ...
            <div>
              <p className="font-medium text-gray-700">현재 신청 가능한 동아리가 없습니다.</p>
              <p className="mt-1 text-sm text-gray-400">새로운 동아리가 등록되면 이곳에 표시됩니다.</p>
            </div>
          </div>
        )}
        ...
        {!isLoading && !isError && clubs.length > 0 && (
          <>
            <p className="mb-4 text-sm text-gray-500">
              총 <span className="font-semibold text-blue-600">{clubs.length}</span>개의 동아리
            </p>
